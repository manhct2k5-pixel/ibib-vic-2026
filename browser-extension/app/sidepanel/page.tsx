...
export default function SidePanelPage() {
  ...
  if (!authenticated) return <div className={`panel-shell auth-shell ${typeof window !== 'undefined' && window.self !== window.top ? 'embedded-panel' : ''}`}><header><button className="brand brand-link" type="button" title="Truy cập trang chủ" onClick={() => void openHomepage()}><i>◆</i><div><strong>IBIB</strong><small>Trợ lý tuân thủ</small></div></button></header><main className="auth-gate"><i>AI</i><h1>Đăng nhập để sử dụng</h1><p>Truy cập trang chủ để đăng nhập hoặc đăng ký. Tiện ích sẽ tự đồng bộ tài khoản và quyền sử dụng của bạn.</p><button type="button" onClick={() => void startLogin()}>Truy cập trang chủ</button><small>Quay lại tiện ích sau khi đăng nhập thành công.</small></main></div>

  return <div className={`panel-shell ${typeof window !== 'undefined' && window.self !== window.top ? 'embedded-panel' : ''}`}>
    <header><button className="brand brand-link" type="button" title="Truy cập trang chủ" onClick={() => void openHomepage()}><i>◆</i><div><strong>IBIB</strong><small>Trợ lý tuân thủ</small></div></button></header>
